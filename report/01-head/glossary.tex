...
\newglossaryentry{enc-dec}
{
  ...
  description={In \glsfirst{ml}, Encoder-Decoder are two \glspl{nn} that work in pair. The Encoder generates a fixed-size output vector from any sized vector input, and the Decoder generates from the Encoder output a vector that could be any size}
}
...
{
  name={Multi-Hop},
  description={In \glsfirst{qa} Systems, a Multi-Hop implies that the answer is within multiple \gls{hop} of the question. In other words, the answer requires a combination of different information to be answerable. Generally, extra qualifying Subject-Predicate-Object Tuples (SPOs) are separating the question Subject and the answer Object}
}
...
{
  name={Open Domain},
  description={In \glsfirst{ir}, the support of Open Domain questions provides no restriction to the questions' subject}
}
...
\newglossaryentry{gmdp}
{
  ...
  description={In the context of the \glsfirst{rl}, this process models the ability to predict the next state of a finite-state machine-like process, such as a game, with only the information contained in the present state}
}
...
\newglossaryentry{grl}
{
  ...
  description={In \glsfirst{ml}, this type of learning combines generally a \glsfirst{mdp} environment with an approach similar to \glsfirst{ul} as it does not require labelled data. The particularity of this technique is that it uses a notion of rewards to predict the best next-step by running a large amount of simulations as training}
}
...
\newglossaryentry{gul}
{
  ...
  description={In \glsfirst{ml}, this type of learning implies the use of unlabelled datasets to perform the training}
}
...
\newglossaryentry{gal}
{
  ...
  description={In \glsfirst{ml}, the concept of this technique relies on trying to fool models via malicious inputs. Interpretable as a game a model plays with itself, in which the model modifies the input in such a way that it will recognize it as another input, and then learn from its mistake}
}
...
\newglossaryentry{gmu}
{
  ...
  description={In \glsfirst{ml}, Machine Understanding stays ambiguous in its definition. However, we use the term as the ability to represent knowledge as atomic building blocks and fundamental relations}
}
...
{
  name={BLEU},
  description={In \glsfirst{nlp}, The Bilingual Evaluation Understudy (BLEU) is an evaluation metrics particularly popular in machine translation as it is an automatic processing}
}

\newglossaryentry{oracle}
{
  name={Oracle},
  description={In \glsfirst{ml}, an Oracle is defined by an entity who knows that knows the ground truth to all questions. It can be a human, or an algorithm querying perfectly a database}
}
